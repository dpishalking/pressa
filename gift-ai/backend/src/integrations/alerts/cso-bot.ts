...
function settingsText(chatId: string): string {
  ...
  return [
    ...
    `Счёт выставлен — ${toggleLabel(s.invoiceSent)}`,
    `Оплаты на счёт — ${toggleLabel(s.payments)}`,
...
function helpText(): string {
  return [
    ...
    "/issued on|off — счёт выставлен",
    "/payments on|off — оплаты на счёт",
  ...
  if (command === "/status") {
    ...
    await reply(
      ...
      [
        ...
        `Счёт без оплаты: ${cfg.invoiceMinEur > 0 ? `от ${eur(cfg.invoiceMinEur)}` : "любая сумма"}, ${cfg.invoiceUnpaidDays} дн.`,
        `Оплата на счёт: ${cfg.invoiceMinEur > 0 ? `от ${eur(cfg.invoiceMinEur)}` : "любая сумма"}`,
