...
function parseFxOverrides(raw: string): Record<string, number> {
  ...
  try {
    ...
    throw new Error("ANALYTICS_FX_OVERRIDES должен быть JSON, например {\"BYN\":0.30}");
  }
}

export function analyticsExportEnabled(): boolean {
  return Boolean(
    config.BITRIX24_WEBHOOK_URL &&
      config.ANALYTICS_SHEET_ID &&
      config.GOOGLE_SERVICE_ACCOUNT_JSON &&
      config.ANALYTICS_COUNTRY_TAGS.length,
  );
}

export function analyticsExportConfig(): AnalyticsExportConfig {
  if (!config.BITRIX24_WEBHOOK_URL) {
    throw new Error("BITRIX24_WEBHOOK_URL не настроен");
  }
  if (!config.ANALYTICS_SHEET_ID) {
    throw new Error("ANALYTICS_SHEET_ID не настроен");
  }
  if (!config.GOOGLE_SERVICE_ACCOUNT_JSON) {
    throw new Error("GOOGLE_SERVICE_ACCOUNT_JSON не настроен");
  }
  if (!config.ANALYTICS_COUNTRY_TAGS.length) {
    throw new Error("ANALYTICS_COUNTRY_TAGS не задан — укажите теги стран через запятую");
  }

  return {
    sheetId: config.ANALYTICS_SHEET_ID,
    serviceAccountJson: config.GOOGLE_SERVICE_ACCOUNT_JSON,
    countryTags: config.ANALYTICS_COUNTRY_TAGS,
