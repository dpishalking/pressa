...
function buildUserPrompt(opts: {
  ...
  const userNote = isRepeatRequest(userMessage)
    ...
    : isCatalogQuestion(userMessage)
      ? "Клиент хочет каталог — кратко: подберёшь из каталога после пары базовых вопросов, и задай текущий этап."
      : "";
...
Ответь JSON согласно инструкции. Поле reply — не длиннее 400 символов. Поле reply ОБЯЗАНО заканчиваться вопросом (кроме isComplete=true).`;
}

const SHORT_REPLY_HINT =
  "\n\nВАЖНО: предыдущий ответ был слишком длинным. Поле reply — максимум 300 символов. Без лишних вопросов про хобби и истории. JSON должен быть полным и валидным.";
