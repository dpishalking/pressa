...
export function questionForStage(stage: ConsultationStage, fields: QualificationFields): string {
  switch (stage) {
    ...
    case 4:
      return "Есть примерный бюджет, в который хотелось бы уложиться?";
    case 5:
      return "Что получатель должен почувствовать, когда откроет подарок?";
    case 6:
    ...
    case 9:
      return "Какой из вариантов ближе — или рассказать разницу подробнее?";
    case 10:
      return "Оставьте имя и телефон — передам менеджеру, он поможет оформить.";
    default:
...
Подсказка вопроса: «${questionForStage(next, fields)}»`;
}
